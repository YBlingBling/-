	...
	<body>
		...
		<!-- Header -->
			...
			<div id="header-wrapper">
				 <jsp:include page="./menu.jsp"><jsp:param value="list1" name="type"/> </jsp:include> 
			</div>
							...
							<!-- Sidebar -->
								...
								<section>
										...
										<div id="fd1" style="" class="menu_body">
											...
											<a href="${ctx}/com/queryVote.do?s_quesType.voteType.id=${item.id}">${item.name }</a>
											</c:forEach>
										</div>
								</section>
						...
						<div class="9u mobileUI-main-content">
							...
							<!-- Content -->
									...
									<article class="first" style="margin-top: 1px">
										<form action="${ctx}/com/queryVote.do">
											<select name="s_quesType.voteType.id"  style="height: 32px;font-size: 14px;background: #f1f1f1">
													<option value="">选择主题分类</option>
													<c:forEach items="${voteTypeList }" var="item">
														<option value="${item.id }">${item.name }</option>
													</c:forEach>
												</select>
											<input type="text" name="s_title" placeholder="输入主题名称" style="height: 32px;font-size: 14px" size=40/>
										 ...
										 <table width="100%" style="margin-top: 20px">
										 	...
										 	<c:forEach items="${SESSION_PAGE_WEB.list }" var="item">
										 		...
										 		<td align="left"><a href="${ctx }/com/getVote.do?uid=${item.id}" >${item.title }</a></td>
										 		</tr>
										 	</c:forEach>
										 	</tbody>
										 </table>
...
<script type="text/javascript" >
function checklogin(){
	var replyUsername ="${SimpleUser.id}";
	if(replyUsername==""){
		alert("请先登录");return false;
	}
	location.hash="noteForm";
	return true;
}
function tonote(){
