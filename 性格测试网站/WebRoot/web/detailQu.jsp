			...
			<div class="row">
				...
				<div class="9u mobileUI-main-content">
					<article class="first" style="margin-top: 1px">
						<form action="${ctx }/com/updateQuScore.do" method="post" id="voteform">
							<input type="hidden" name="uid" value="${item.id }" />
							<h3>${item.title}</h3>
							<c:forEach items="${list}" var="p1">
								<h4 style="margin-top: 20px; color: blue">
									<a id="itemTitle_item_${p1.id }">${p1.title }</a>
								</h4>
								<ul>
									<li><input type="radio" name="item_${p1.id }" id="item_${p1.id }" value="${p1.score1 }" /> ${p1.option1 }</li>
									<li><input type="radio" name="item_${p1.id }" id="item_${p1.id }" value="${p1.score2 }" /> ${p1.option2 }</li>
									<li><input type="radio" name="item_${p1.id }" id="item_${p1.id }" value="${p1.score3 }" /> ${p1.option3 }</li>
									<li><input type="radio" name="item_${p1.id }" id="item_${p1.id }" value="${p1.score4 }" /> ${p1.option4 }</li>
								</ul>
							</c:forEach>
