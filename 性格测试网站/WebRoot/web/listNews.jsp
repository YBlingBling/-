...
<body>
	...
	<div id="header-wrapper">
		<jsp:include page="./menu.jsp"><jsp:param value="news" name="type" />
		</jsp:include>
	</div>
	...
	<div id="main-wrapper">
		<div class="5grid-layout">
			<div class="row">
				...
				<!-- Content -->
				<ul style="wdith: 100%;; margin-top: 55px; margin-left: 20px">
					<li>
						<form action="${ctx}/com/queryNews.do" method="post" style="float: right">
							<input placeholder="请输入关键字" name="s_title" size="50">
							<input type="submit" value="查询" class="button medium">
						</form>
						<div style="clear: both"></div>
					</li>
					<c:if test="${empty SESSION_PAGE_WEB.list }">
					...
					<c:forEach items="${SESSION_PAGE_WEB.list }" var="item">
						<li style="wdith: 100%; border-top: 1px solid #e5e5e5;; margin-top: 5px">
							<div style="float: left; display: inline; width: 15%">
								<img alt="" width="95%" src="${ctx }/resource/${item.imageFile}">
							</div>
							<div style="float: left; display: inline; width: 80%">
								<a href="${ctx }/com/getNews.do?uid=${item.id}" target="blank"><span
									style="color: #006030; font-size: 16px; font-weight: bold;">${item.title }</span></a> <br /> ${item.note } <br />
							</div>
						</li>
						<div style="clear: both"></div>
					</c:forEach>
