...
<body>
	...
	<div id="header-wrapper">
		<jsp:include page="./menu.jsp"><jsp:param value="user" name="type" />
		</jsp:include>
	</div>
			...
			<div class="row">
				<div class="3u">
					...
					<section>
						<h3>个人中心</h3>
						<ul class="link-list">
							<li><a href="${ctx }/com/showResult.do">我的人格评测</a></li>
							<li><a href="${ctx }/com/user.do?type=hist">历史记录</a></li>
					...
					<article class="first" style="margin-top: 50px">
							...
							<table width="100%" style="margin-top: 20px">
								<thead>
									<tr style="background: #f1f1f1; font-weight: bold">
										<th align="left" width="70%">主题</th>
										<th align="left" width="20%">时间</th>
									</tr>
								</thead>
								<tbody style="font-size: 13px">
									<c:if test="${empty forumlist }">
										<tr>
											<td colspan="3" style="color: blue">木有找到呢 -_-</td>
										</tr>
									</c:if>
									<c:forEach items="${forumlist}" var="item">
										<tr>
											<td align="left">
												<c:if test="${item.onTop }">
													<span style="color: red">[置顶]</span>
												</c:if>
												<c:if test="${not  empty item.clazz }">[${item.clazz.name}]</c:if>
												<a href="${ctx }/com/getForum.do?uid=${item.id}" target="_blank">${item.title }</a>
											</td>
											<td>${item.addDate }</td>
										</tr>
									</c:forEach>
						...
						<c:if test="${type=='hist' }">
							<table width="100%" style="margin-top: 20px">
								<thead>
									<tr style="background: #f1f1f1; font-weight: bold">
										<th align="left" width="10%">测试日期</th>
										<th align="left" width="20%">测试分数</th>
										<th align="left" width="20%">测试结果</th>
									</tr>
								</thead>
								<tbody style="font-size: 13px">
									...
									<c:forEach items="${mylist}" var="item">
										<tr>
											...
											<td>${item.score }</td>
											<td align="left">
												<a href="${ctx }/com/getOneResult.do?uid=${item.id}">${item.personality.name }</a> 
												<a href="${ctx }/com/delUserResult.do?uid=${item.id}"
													class="button small orange">删除</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
						...
						<c:if test="${type=='info' }">
							<form action="${ctx }/com/userUpdate.do" method="post" class="basic-grey" id="updateForm"
								style="margin-left: 50px">
								<label>
									<span>账号 :</span>
									<input id="regname" type="text" value="${SimpleUser.user.uname }" readonly="readonly" name="uname"
										placeholder="您的账号" />
								</label>
								<label>
									<span>姓名 :</span>
								...
								<label>
									<span>生日 :</span>
									<input id="regphone" type="text" name="userBirth" value="${SimpleUser.user.userBirth}"
										placeholder="您的生日.如:1990-08-08" />
								</label>
								<label>
									<span>联系电话 :</span>
								...
								<label>
									<span>联系地址 :</span>
									<input id="regaddress" type="text" name="userAddress" value="${SimpleUser.user.userAddress }"
										placeholder="您的联系地址" />
								</label>
								<label>
									<span>&nbsp;</span>
							...
							<tbody style="font-size: 13px">
								...
								<c:forEach items="${mylist}" var="item">
									<tr>
										...
										<td align="left">
											<a href="${ctx }/com/getUserVote.do?uid=${item.id}">${item.title }</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
							...
							<tbody style="font-size: 13px">
								...
								<c:forEach items="${mylist}" var="item">
									<tr>
										...
										<td align="left">
											<a href="${ctx }/com/getUserQues.do?uid=${item.id}">${item.name }</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
