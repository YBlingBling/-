...
<article class="first" style="margin-top: 1px;">
	...
	<pre style="font-size: 12px">${currentResult.personality.note}</pre><br/>
	
	<c:if test="${not empty preResult }">
	<br/>
	<h3> 历史评测结果: [${preResult.personality.name}]</h3>
	<pre style="font-size: 12px">${preResult.personality.note}</pre><br/>
	</c:if>
	<c:if test="${not empty trans }">
	<br/>
	<h3> 人格转换评测: </h3>
	<pre style="font-size: 12px">${trans.note}</pre><br/>
	</c:if>
</article>
</div>
					</div>
